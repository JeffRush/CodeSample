...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Import_JnJ_Step9c_ApplicantsUpdate]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Import_JnJ_Step9c_ApplicantsUpdate]
GO

/****** Object:  StoredProcedure [dbo].[Import_JnJ_Step9c_ApplicantsUpdate]    Script Date: 12/04/2016 17:03:32 ******/
SET ANSI_NULLS ON
GO
...
CREATE proc [dbo].[Import_JnJ_Step9c_ApplicantsUpdate]
(
@BatchID as int
)
WITH RECOMPILE 
...
update FACT_Applicant 

set MissingMultiHireReqApplicants =
	case when (select COUNT(*) 
			from FACT_Applicant fa2 where fa2.Employer_Key = fa.Employer_Key
				and fa2.PlanYear = fa.PlanYear
				and fa2.SnapShotDate = fa.SnapShotDate
				and fa2.RequisitionNumber = fa.MultiHireReqNo
				) = 0 then 1
			else 0
		end
...
where fa.MultiHireReqNo is not null
	and fa.MultiHireReqNo != ''
